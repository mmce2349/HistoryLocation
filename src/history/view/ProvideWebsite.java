...
// 		    private JEditorPane displayEditorPane;
		   ...
		   private ArrayList pageHistoryList = new ArrayList();
// 		    private ArrayList pageList = new ArrayList();
...
// 		        });
		 ...
		 JMenu fileMenu = new JMenu("File");
		 fileMenu.setMnemonic(KeyEvent.VK_F);
		 JMenuITem fileExitMenuItem = new MenuItem("Abort Program", KeyEvent.VK_x);
		 fileExitMenuItem.addActionListener(new ActionListener()
		 {
		 public void actionPerformed(ActionEvent e)
		 {
...
// 		        });
	...
	buttonPanel.add(backButton);
	forwardButton = new JButton(" Forward ->);
	forwardButton.addActionListener(new ActionListenr()
	{
		public void actionPerformed(ActionEvent event)
		{
			actionForward();
...
// 		    }
		  ...
		  private void actionBack()
		  {
			...
			try
			{
				showPage(newURL((String) pageList.get(pageIndex -1)), false);
			}
			catch(Exception e)
			{
		...
		private void showPage(URL pageUrl, boolean addToList)
		{
			setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
				try
				{
					URL currentURL = displayEditorPane.getpage();
					displayEditorPane.setPage(pageUrl);
					URL newUrl = displayEditorPage.getPage();
					//abstraction
					if(addToList)
					{
						int listSize = pageList.size();
...
// 		//startComplexity
		...
		if (eventType == HyperlinkEvent.EventType.ACTIVATED)
		{
			if(event instanceof HTMFrameHyperlinkEvent)
			{
				HTMLFrameHyperlinkEvent linkEvent = (HTMLFrameHyperlinkEvent) event;
				HTMLDocument document = (HTMLDocument) displayEditorPane.getDocument();
				document.processHTMLframeHyperlinkEvent(linkEvent);
...
// 		        browser.show();
// 		    }
>>>>>>> 257b795e48168b1e7930e6c2a4c593c6cfbb5162
}
